...
async function saveUser(){
  ...
  async function viewRoom(){
    let {data} = await axios.get(`http://192.168.1.7:3000/patient/viewRoom`,{
  ...
  async function getRoom(patientId,roomId){
    let {data} = await axios.get(`http://192.168.1.7:3000/patient/BookRoom?userID=${patientId}&roomID=${roomId}`,{
      headers:{token:`${await AsyncStorage.getItem('Token')}`}
    }).catch((errr)=>{
      console.log(`${errr.response.data.Error}`)
       })
